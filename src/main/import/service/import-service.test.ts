...
const POSTMAN_COLLECTION_FILE_PATH = 'path/to/postman-collection.json';
...
describe('ImportService', () => {
  it('should import a Postman collection', async () => {
    ...
    (PersistenceService.instance.storeCollection as jest.Mock).mockImplementation(async (collection: Collection) => collection);
    ...
    // Assert
    await expect(fs.readFile(POSTMAN_COLLECTION_FILE_PATH)).resolves.toBe(POSTMAN_COLLECTION);
    ...
    const firstChild = childrenLevel1[0] as Request;
    expect(firstChild).toBeInstanceOf(Request);
    ...
    const secondChild = childrenLevel1[1] as Request;
    expect(secondChild).toBeInstanceOf(Request);
    ...
    expect(thirdChild).toBeInstanceOf(Folder);
    expect(thirdChild.title).toBe('This is a folder');

    const childrenLevel2 = thirdChild.children;
    expect(childrenLevel2.length).toBe(4);

    const firstChildLevel2 = childrenLevel2[0] as Request;
    expect(firstChildLevel2).toBeInstanceOf(Request);
    ...
    const secondChildLevel2 = childrenLevel2[1] as Request;
    expect(secondChildLevel2).toBeInstanceOf(Request);
