...
describe('calculateResponseSize', () => {
  ...
  it('calculates size correctly with multiple header values', () => {
    const headers = { 'set-cookie': ['cookie1=value1', 'cookie2=value2'], 'content-type': 'application/json' };
    const result = calculateResponseSize(headers);
    expect(result).toEqual({
      totalSizeInBytes: 76,
  ...
  it('calculates size of header correctly for several cases combined', () => {
    const headers = {
      ...
      'Content-Security-Policy-Report-Only': 'object-src \'none\';base-uri \'self\';script-src \'nonce-PiU4uUsxJHMfI86u0fq6eA\' \'strict-dynamic\' \'report-sample\' \'unsafe-eval\' \'unsafe-inline\' https: http:;report-uri https://csp.withgoogle.com/csp/gws/other-hp',
      'Server': 'gws',
      'X-XSS-Protection': '0',
      'X-Frame-Options': 'SAMEORIGIN',
      'Set-Cookie': ['AEC=AVYB7cpFDNfvuiVCfhVB5CNosUmOPykRrb9AaXhY-Erip_-YyWGHFC5gPMc; expires=Sun, 20-Apr-2025 08:47:51 GMT; path=/; domain=.google.de; Secure; HttpOnly; SameSite=lax'],
      'Accept-Ranges': 'none',
